...
def hometown(town):
    """Given a town, evaluates if it is the correct town"""
    if town == "Recife":
        correct_town = True
    else:
        correct_town = False
...
my_hometown = hometown("Recife")
#    (b) Write a function that takes a first and last name as arguments and
#        returns the concatenation of the two names in one string.
def concatenate_names(first_name, last_name):
    """Given a first and last name, returns full name concatenated"""
    full_name = "%s" " " "%s" %(first_name, last_name)
    return full_name

given_name = concatenate_names("Letania", "Ferreira")
...
def gretings_from_same_town(full_name, correct_town):
    """given a home town and a full name, greets people and comment on town"""
    if correct_town == True:
        greetings = "Hi %s, we're from the same place!" %(full_name)
    else:
        greetings = "Hi %s, I'd like to visit %s!" %(full_name, correct_town)
    return greetings

gretings_from_same_town(given_name, my_hometown)
...
def calculate_price(base_price, state='CA', taxes=.05):
    ...
    """
    tax_amount = base_price * taxes
    price_with_taxes = base_price + tax_amount
    ...
    if state == "MA":
        ...
        fee_amount = price_with_taxes * california_fee
    else:
        fee_amount = 0

    total_price = price_with_taxes + fee_amount
    return total_price
...
original_lst = [1, 56, 78, 4]
new_lst = []

def append_n_arguments_to_list(lst, *arg):
    for item in original_lst:
        new_lst.append(item)

    new_lst.append(len(arg))

    return new_lst
append_n_arguments_to_list(original_lst, "house", "ball")
